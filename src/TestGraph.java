...
public class TestGraph {
	public static void main(String[] argv) {
		...
		for(String player : players) {
			...
			Set<Route> routes = allRoutes.stream().filter(r -> r.getOwnedByPlayerID().equals(player)).collect(Collectors.toSet());
			...
			for(DestCard card : p1Dest) {
				if(!(g.containsVertex(card.startCity) && g.containsVertex(card.endCity))) {
					continue;
				}
				if(ci.pathExists(card.startCity, card.endCity)) {
					cardPoints += card.pointValue;
				}
			}
		}
	}
